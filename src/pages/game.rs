...
// write to spectrogram
// have the big file with all the text
// make the other file with the hints (which will be uploaded)
// upload all the media
// change the titles of some of the things
//  put the website up
//  
...
#[component]
fn Image(src: &'static str) -> impl IntoView {
    view!{
        <img
            src=src
            />
...
fn Section1(on_phrase_click: impl Fn(leptos::ev::MouseEvent) + 'static, title: &'static str) -> impl IntoView {
    view!{
        ...
        <div> Puzzles in video games suck. Something that should be used to reward the player for <Clickable text_content="thoughtfulness" content=|| view!{<Image src="website/media/section1.png"/>}/> is more often than not used as a lazy mechanic to explore other areas. The amount of times that Ive seen a <Clickable text_content="colored lock" content=|| view!{<Image src="website/media/section1_2.jpg"/>}/> only to have to  <button style:all="unset" on:click=on_phrase_click>lope</button> around on an excursion to find a key of matching color, completely ignoring the story thats being told in order to get to the next section of the game. I can only stand so many -- what seems like -- fetch quests in games without it feeling drawn out and unnecessary to the development of whatever is going on.</div>
    }
}
        ...
        view! {
            {
                move || match *state.read() {
                    Self::Begining => view! {   
                        ...
                        <div> It seems as though interacting with some of the words / phrases in the text is hinting at something but I cant seem to figure it out </div>
                    ...
                    Self::Section1(count) => {
                        view!{
                            <Section1 on_phrase_click=update_section_1_count(set) title="Introduction" />
                        }.into_any()
                    }
                    ...
                    Self::Section3(replaying, found) => {
                        ...
                        if let ReplayingState::Replaying(count) = replaying {
                            view!{
                                <Section1 on_phrase_click=update_section_3_count(set) title="This whole thing again ?" />
                            }.into_any()
                        } else {
                            view!{
                                <Title content=move || view!{ <div on:click=move |_| if matches!(replaying, ReplayingState::Finished) {set(GameState::Section4(vec![]))} else if !found { set(GameState::Section3(replaying, true))}><Header content="Nonlinearity & Replayability"/></div>} />
                    ...
                    Self::Section5(ref phrase, back) => {
                        ...
                        if back {
                            view!{
                                <Title content=|| view!{ <Header content="Backtracking !"/>} />
                    ...
                    Self::End => view! {
                        ...
                        <div>the full document can be found <a href="https://docs.google.com/document/d/1r4luGtJQDJzz4nHziwTRzj462i5bBAL6bTD58oI_Q80/edit?usp=sharing" target="_blank">Here</a> if you arent sick of reading it already</div>
                    }.into_any()
                }
            }
        }
...
fn clear_events() -> impl Fn(leptos::ev::MouseEvent) {
    |ev| {
        ev.prevent_default();
...
fn Clock(signal: WriteSignal<GameState>) -> impl IntoView {
    ...
    let on_click = move |ev: leptos::ev::MouseEvent| {
        ...
        if dist as i32 > radius || dist < (radius as f32) * (1./3.) {
            return;
        }
    ...
    view!{
        ...
        >
            ...
            >
                <Image src="website/media/clock.jpg"/>
            </div>
        </div>
    }
}
